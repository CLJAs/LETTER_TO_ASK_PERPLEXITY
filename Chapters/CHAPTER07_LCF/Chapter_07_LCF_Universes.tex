\chapter{LCF and Universes $\theta_{k}$  }
	...
	\section{Initial concepts needed}
		...
		\noindent
		A CF is a t-upla of two terms. The left term is a SNEF, ALWAYS a SNEF. The right term is just a natural number, a DR value, that we put inside another t-upla of one member, to simplify the description of the bijection $\Omega$. In that way we keep all terms in the domain of t-uplas of natural numbers.
		\\\\
	...
	\section{Second partition: $LCF_{1}$ and $LCF_{2}$ }
		...
		\noindent
		One member of LCF belongs to $CLF_{1}$ if it only has one CF inside its list.
		\\\\
	...
	\section{Third partition: $LCF_{2c}$ and $LCF_{2p}$ }
		...
		1) Both SNEFs, inside the two CFs, are initial sequences of one SNEI. No matter if you can find two different SNEIs, for whom you can find, the two SNEFs, are initial sequences of one, and the another. ONCE you can find a singular SNEI, and both SNEFs are initial sequences of it, THAT member belongs to $LCF_{2p}$.\\\\
	...
	\section{Fourth partition: Universes $\theta_{k}$ }
		...
		\noindent
		NOW you have, one of the points you asked me. Each relation $r_{\theta_{k}}$, is going to use, the universe $theta_{k}$ as Image set, and the entire set SNEIs, as its Domain set. Now you know what are the universes.
		\\\\
		
		\noindent
		You can easily imagine the bijections between the subsets, and paths in the general scheme \ref{fig:gereral_scheme}. BUT I need to write it. I am using your letter to do what I promised in an old document published in Vixra. So let me do it, the most perfectly, that I could. You can ignore the next two chapters, now you know EXACTLY what are the SNEIs, and what is $LCF_{2p}$, and universes $\theta_{k}$.
		\\\\
