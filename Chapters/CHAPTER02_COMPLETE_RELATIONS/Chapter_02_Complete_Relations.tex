\chapter{Complete Relations: D-pair, R-pair, PACK}
	...
	\noindent
	Our relations could have more than one image element, for each element of the Domain. AND, they can have repetitions. They can have the same element of the image set assigned to different elements of the Domain. The golden rule for this relations is that: \textbf{EVERY ELEMENT OF THE DOMAIN MUST HAVE, AT LEAST, ONE IMAGE}: no matter how... even cheating, repeating the use of elements of the Image set of the relation. They are called COMPLETE RELATIONS, because they always try to cover the Domain ``completely''.
	\\\\
	
	\noindent
	\textbf{R-pair}: means a pair of the relation. A t-upla of two elements, where left element belongs to the Domain, and the right element belongs to the Image.
	\\\\
